...
def get_args( ):
    ...
    # main arguments
    attach_common_arguments( parser, no_output=True )
...
def generate_colormap( name, reference, items, overrides=None ):
    ...
    if reference is not None and os.path.exists( reference ):
        say( "Reading {} colors from file: {}".format( name, reference ) )
    ...
    # too many colors to pick them automatically
        ...
        die( "Can't auto-color >18 {} taxa".format( name ) )
    return color_dict
...
class BarplotTable:
    ...
    def update( self ):
        ...
        if not (self.nrows == len( self.rowheads ) == len( self.rowmap )):
            die( "Row dimension mismatch" )
        if not (self.ncols == len( self.colheads ) == len( self.colmap )):
            die( "Col dimension mismatch" )
...
class BarplotLegend( ):
    ...
    def check( self ):
        if self.cdex + 1 > self.cols:
            die( "Ran out of legend space. Increase rows/cols or label fewer things." )
...
def main( ):
    ...
    for method in args.sort:
        if "braycurtis" in method and not args.remove_zeros:
            die( "Can't sort by <{}> without invoking <--remove-zeros>".format( method ) )
